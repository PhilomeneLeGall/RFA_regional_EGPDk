###########################################################################
##### Semi-regional inference of EGPD on ERA5 precipitation data (EU) #####
#####                         All Gridpoints                          #####
...
nc_clusters <- nc_open(paste0(output_directory,"Omega_Spatial_clustering/Clustering_procedure/Output_clustering/Omega_clusterpam_10_ERA5_EU_", seas, ".nc"))
...
List_semireg_fit <- list() #store the fitting parameters
List_keep_coord_semi <- list() #keep in memory the coordiantes of the gridpoint
for(clust_nb in 1:nclust){ #loop on the clusters
  ...
  for (LON in 1:num_lon) {
    for (LAT in 1:num_lat) {
        if(!is.na(optimal_partition[LON,LAT]) & optimal_partition[LON,LAT]==clust_nb){ #check we have data and the GP is indeed in the good cluster
          ...
          ind_1_third <- (1:length(y))[which(((1:length(y))%%3)==0)]#Only keep one third of precipitation time series to get rid of autocorrelation
  ...
  List_keep_coord_semi[[clust_nb]] <- list(keep_lon = keep_lon, keep_lat = keep_lat)
  List_semireg_fit[[clust_nb]] <- fitEGPDkSemiRegCensoredIter_paral(M = t(M_pos_precip), ncores = floor(detectCores()/3 - 1), cens_thres = c(1,Inf))
}#end for clust_nb

save(List_semireg_fit, file = paste0(output_directory,"res_semireg_fit_PAM_",nclust,"clus_allGP_",seas,"_1third.Rdata"))
save(List_keep_coord_semi, file = paste0(output_directory,"keep_coord_semireg_fit_PAM_",nclust,"clus_allGP_",seas,"_1third.Rdata"))
